...
export default function FilterForms(props) {
    ...
    let filters = [];
    let saveSegmentDisable = false;

    function radioButtonHandleInput(set) {
        let setName = set.target.name;

        if (setName === "customer_acceptance") {
            dataFromFilters.acceptsMarketing = set.target.value;
        }

        //Payment Status
        if (setName === "payment_status_paid") {
            dataFromFilters.payment_status_paid = set.target.value;
        }
    }
    ...
    function handleTextBoxInput(set) {
        ...
        if (setName === "aov_min") {
            dataFromFilters.aov_min = set.target.value;
        }

        if (setName === "aov_max") {
            dataFromFilters.aov_max = set.target.value;
        }

        if (setName === "last_ordered_days") {
            dataFromFilters.last_ordered_days = parseInt(set.target.value);
        }

        if (setName === "min_spend_total") {
            dataFromFilters.min_spend_total = parseFloat(set.target.value);
        }
    }
    ...
    function dateHandleInput(set) {
        ...
        if (setName === "signed_up_start_date") {
            dataFromFilters.signed_up_start_date = set.target.value;
        }

        if (setName === "signed_up_end_date") {
            dataFromFilters.signed_up_end_date = set.target.value;
        }
    }
    ...
    function generateDataForServer() {
        //AOV CONDITION
        if (dataFromFilters.aov_min !== "" && dataFromFilters.aov_max !== "") {
            filters.push({
                name: "AOV",
                data: {
                    aov_min: dataFromFilters.aov_min,
                    aov_max: dataFromFilters.aov_max,
                },
                display: `AOV(Total) BETWEEN ${dataFromFilters.aov_min} and ${dataFromFilters.aov_max}`,
            });
            console.log(customFilters);
        }

        //acceptsMarketing
        if (dataFromFilters.acceptsMarketing !== "") {
            if (dataFromFilters.acceptsMarketing === "accepted") {
                filters.push({
            ...
            if (dataFromFilters.acceptsMarketing === "notAccepted") {
                filters.push({
                    name: "acceptsMarketing",
                    data: false,
            ...
            console.log(filters[0].display);
        }

        //last_ordered_days
        if (dataFromFilters.last_ordered_days !== "") {
            filters.push({
                query: "last_ordered_days =" + dataFromFilters.last_ordered_days,
                display: "Last Ordered",
            });
        }

        //min_spend_total
        if (dataFromFilters.min_spend_total !== "") {
            filters.push({
                query: "totalSpentV2_amount >= " + dataFromFilters.min_spend_total,
                display: "Min. Spend Total",
            });
        }

        //Payment Status
        if (dataFromFilters.payment_status_paid !== "") {
            if (dataFromFilters.payment_status_paid === "fullyPaid") {
                filters.push({
                    query: "payment_status_paid = true",
                    display: "Fully Paid",
                });
            }
            if (dataFromFilters.payment_status_paid === "notFullyPaid") {
                filters.push({
        ...
        //signed_up;
        if (dataFromFilters.signed_up_start_date !== "") {
            filters.push({
                query: "signed_up_start_date =" + dataFromFilters.signed_up_start_date,
                display: "Signed up Start Date",
            });
        }
        if (dataFromFilters.signed_up_end_date !== "") {
            filters.push({
                query: "signed_up_end_date =" + dataFromFilters.signed_up_end_date,
        ...
        if (filters.length !== 0) {
            handleDisableButton(saveSegmentDisable);
        }
        onHide();
    }
    ...
    return (
        <div>
            <Modal show={showCustomFilter} size="lg" aria-labelledby="contained-modal-title-vcenter" centered>
                <Modal.Body>
                    <Row>
                        <Box sx={{ flexGrow: 1, bgcolor: "background.paper", display: "flex", height: 400 }}>
                            <Tabs orientation="vertical" variant="scrollable" value={value} onChange={handleChange} aria-label="Vertical tabs example" sx={{ borderRight: 1, borderColor: "divider" }} className={styles.vertical_tabs}>
                                <Tab label="AOV(Total)" {...a11yProps(0)} className={styles.tabText} />
                                <Tab label="Accepts Marketing" {...a11yProps(1)} className={styles.tabText} />
                                <Tab label="Account State" {...a11yProps(2)} className={styles.tabText} />
                                <Tab label="Customer Tag" {...a11yProps(3)} className={styles.tabText} />
                                <Tab label="Last Ordered" {...a11yProps(4)} className={styles.tabText} />
                                <Tab label="Min. Spend(Total)" {...a11yProps(5)} className={styles.tabText} />
                                <Tab label="Order Source" {...a11yProps(6)} className={styles.tabText} />
                                <Tab label="Orders Total" {...a11yProps(7)} className={styles.tabText} />
                                <Tab label="Signed Up" {...a11yProps(8)} className={styles.tabText} />
                                <Tab label="First Purchase Product" {...a11yProps(9)} className={styles.tabText} />
                                <Tab label="First Purchase Variant" {...a11yProps(10)} className={styles.tabText} />
                                <Tab label="Product Tag" {...a11yProps(11)} className={styles.tabText} />
                                <Tab label="Product Title" {...a11yProps(12)} className={styles.tabText} />
                                <Tab label="Product Type" {...a11yProps(13)} className={styles.tabText} />
                                <Tab label="Product Variant" {...a11yProps(14)} className={styles.tabText} />
                                <Tab label="Vendor" {...a11yProps(15)} className={styles.tabText} />
                                <Tab label="Country" {...a11yProps(16)} className={styles.tabText} />
                                <Tab label="Discount Code" {...a11yProps(17)} className={styles.tabText} />
                                <Tab label="Payment Status" {...a11yProps(18)} className={styles.tabText} />
                                <Tab label="Payment Gateway" {...a11yProps(19)} className={styles.tabText} />
                                <Tab label="Order Paid Status" {...a11yProps(20)} className={styles.tabText} />
                                <Tab label="Order Tag" {...a11yProps(21)} className={styles.tabText} />
                                <Tab label="Province(State)" {...a11yProps(22)} className={styles.tabText} />
                            </Tabs>
                            ...
                            <TabPanel value={value} index={0} className={styles.tab_panel}>
                                ...
                                <input
                                    ...
                                    defaultValue={dataFromFilters.aov_min}
                                ...
                                <input
                                    ...
                                    defaultValue={dataFromFilters.aov_max} placeholder="Enter Amount"
                                    className={styles.rounded_input_corners} />
                            </TabPanel>
                            ...
                            <TabPanel value={value} index={2} className={styles.tab_panel}>
                                ...
                                <div>
                                    ...
                                    // defaultValue={dataFromFilters.account_state_declined}
                                    ></input>
                                    <span>Declined</span>
                                    <hr />
                                    <input
                ...
                <Modal.Footer>
                    ...
                    <Button onClick={generateDataForServer} className={styles.customFilter_applyButton}>
                        Apply
                    </Button>
                </Modal.Footer>
            </Modal>
