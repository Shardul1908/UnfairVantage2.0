...
function Index() {
  ...
  //function to check if start date in date range is mentioned or not
  ...
  //function to check if end date in date range is mentioned or not
  ...
  React.useEffect(() => {
    axios
      .post("http://localhost:8081/api/fetch_saved_segments", {
        shop: shop,
      })
      .then((res) => {
        setSegment(res.data);
          ...
          <div className={styles.save_segments_cards}>
            <Card className={styles.cardStyle}>
              ...
              <Card.Body style={{ color: "white", fontSize: "13px" }}>
                <span>CUSTOMER COUNT: {card.number_of_customers}</span>
                <br />
                <br />
                ...
                <div>
                  <span>START DATE: {card.start_date}</span>
                  <br />
                  <br />
                </div>
                <div>
                  <span>END DATE: {card.end_date}</span>
                  <br />
                  <br />
                </div>
              </Card.Body>
